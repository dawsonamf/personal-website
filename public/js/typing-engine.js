 ...
 * blinking caret. The build-rendered theme can select a cursorless mode with
 * data-typing on <html>:
 ...
 *
 * Deletion normally backspaces one glyph per tick in every mode. A theme can
 * additionally set data-typing-delete="word" on <html> to erase the way
 * 'word' mode types: the
 ...
 *
 * Canonical owner and lifecycle: page code calls startTypingSequence(config) for its
 * masthead. Inputs are the target element/config plus <html> data-typing and
 * data-typing-delete; dependencies are the DOM, fonts, stylesheet load events and timers.
 * A new call cancels the prior run's guarded timers; drain timers may finish removing their
 * own nodes. The page has no SPA teardown, and document unload owns remaining listeners/timers.
 */
function startTypingSequence(config) {
  ...
  // A later caller cancels the prior run before rebuilding the owned element.
  ...
  const mode = config.mode || document.documentElement.dataset.typing || 'cursor';
  ...
  // Captured once, like `mode`, so the current phrase finishes in the grammar
  // selected for this document and call. Word deletion only exists in the
  // cursorless modes — cursor-mode text is
  // bare text nodes with nothing to group.
  const deleteMode = config.deleteMode || document.documentElement.dataset.typingDelete || 'char';
  ...
  // the page and always calls the current run's version, so a later caller does
  // not stack another listener.
  if (document.fonts && document.fonts.ready && document.fonts.ready.then) {
    document.fonts.ready.then(function () { if (!run.cancelled) reserveHeight(); });
  }
  // The type being measured is the skin's, and ThemeAssets emits the skin sheet as
  // a <link data-style-asset> in <head>. Warm in the HTTP cache it applies before
  // these deferred scripts run; cold, it can land after them, and the reservation
  // gets made against the default sheet's 70px/1.2 type. The masthead column is bottom-anchored (css/styles.css), so a
