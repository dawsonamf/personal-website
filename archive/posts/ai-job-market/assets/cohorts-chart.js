(function () {
  ...
  const archiveDataUrl = name => scriptUrl ? new URL('../data/' + name, scriptUrl).href : '../data/' + name;
  ...
  // ../scripts/prebake-cohort-data.py):
  ...
  const charts = [
    makeCohortChart({
      ...
      dataUrl: archiveDataUrl('cohort-unemployment-data.json'),
      series: [
      ...
      uirev: 'cohort-unemp',
      initialMonths: 120,
      overlay: { dataUrl: archiveDataUrl('rate-data.json'), json: 'dfii10', name: '10-year real rate' },
    }),
    makeCohortChart({
      id: 'cohort-swe-chart',
      statId: 'cs-stat',
      toolbar: '.jm-toolbar[data-chart="cs"] ',
      dataUrl: archiveDataUrl('cohort-swe-age-data.json'),
      series: [
    ...
    // HTML block in ../draft.md is commented out too). Kept for easy
    // restore if the overlay experiment doesn't stick.
    // makeCohortChart({
    //   id: 'rate-chart',
    //   statId: 'rate-stat',
    //   toolbar: '.jm-toolbar[data-chart="rate"] ',
    //   dataUrl: archiveDataUrl('rate-data.json'),
    //   series: [
