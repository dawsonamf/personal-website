...
Canaries is a zip; FRED has no CORS) and commits them as small static JSONs:
  ...
  archive/posts/ai-job-market/data/cohort-unemployment-data.json  (NY Fed, monthly, % rates)
  archive/posts/ai-job-market/data/cohort-swe-age-data.json       (Canaries/ADP, monthly, index)
  archive/posts/ai-job-market/data/rate-data.json                 (FRED DFII10, monthly avg, %)

Stdlib only (xlsx is a zip of XML — parsed by hand; no openpyxl). Rerun any
time to refresh the snapshots:
  python3 archive/posts/ai-job-market/scripts/prebake-cohort-data.py
"""
