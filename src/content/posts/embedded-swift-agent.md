...
Update: I added WebAssembly as a compilation target, so now you can [run it live in your browser](/subsites/dawson/embedded-swift-agent/). It's the same Swift code, the only changes are the os level interop: a fetch() bridge instead of libcurl and using JSPI to suspend the (synchronous) agent loop whenever the browser needs to await something. It runs on Chrome and Edge 137+. It has no sh, glob, or grep tools (WASI preview 1 has no process spawning) but it does have access to an in tab memory filesystem for the read and write tools. Tip: ask it about me, my projects, etc. Have fun chatting!
