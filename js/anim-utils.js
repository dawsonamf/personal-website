 ...
 * sizes. Live style switches re-split.
 *
...
(function () {
  ...
  // .section-header-text, optionally exploding it into glyph spans. Runs
  // again on every style switch, so the untouched title is stashed on the
  // node the first time through and re-read from there afterwards.
  ...
  function resplit() {
    ...
    for (var i = 0; i < headers.length; i++) {
      ...
      // A skin switch can change the entrance's length under a header that
      // is already showing.
  ...
  function init() {
    ...
    // Re-split when the skin changes under us (js/theme-cycler.js flips
    // data-style on <html>), so the incoming skin gets the grammar it styles.
    if (window.MutationObserver) {
      new MutationObserver(resplit).observe(document.documentElement, {
        attributes: true,
